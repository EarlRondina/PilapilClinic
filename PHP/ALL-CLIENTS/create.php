...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validate form data
    if (empty($EmergencyP) || empty($AppointmentTime) || empty($EmergencyN) || empty($FirstName) || empty($LastName) || empty($MiddleInitial) || empty($BirthDate) || empty($age) || empty($Gender) || empty($Email) || empty($PhoneNumber) || empty($Department) || empty($Procedure) || empty($AppointmentDate) || empty($BookingType) || empty($Status)) {
                ...
                if ($email_count > 0) {
                    ...
                    if ($phone_count > 0) {
                        ...
                        if ($result1) {
                            ...
                            $sql2 = "INSERT INTO patient (PatientID, EmergencyContactPhone, EmergencyContactName, Department, Procedures, AppointmentDT, AppointmentTime, BookingType, Status)
                                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                            $stmt2 = mysqli_prepare($conn, $sql2);
                            mysqli_stmt_bind_param($stmt2, "issssssss", $patientID, $EmergencyN, $EmergencyP, $Department, $Procedure, $AppointmentDate, $AppointmentTime, $BookingType, $Status);
...
<body>
    <div class="container">
        ...
        <form method="post">
            ...
            <div class="row mb-3">
                <label class="col-sm-3 col-form-label">Emergency Person</label>
                <div class="col-sm-6">
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-6">
                    <input type="text" class="form-control" name="EmergencyN" value="<?php echo $EmergencyN; ?>">
                </div>
            </div>
